...
public class PayController {
	...
	@PostMapping("/use")
	public ResponseEntity<?> usePoint(@RequestBody Map<String, Object> req,
	                                  HttpServletRequest request) {
	    ...
	    try {
	        ...
	        // 3. 요청 body에서 유저 ID 추출
	        Long userid = ((Number) req.get("id")).longValue();
	        if (userid == null || !tokenUserId.equals(userid)) {
	            return ResponseEntity.status(403).body(Map.of("error", "유저 인증 정보 불일치"));
	        }
	        ...
	        int point = ((Number) req.get("point")).intValue();
	        if (point <= 0) {
	            return ResponseEntity.badRequest().body(Map.of("error", "차감할 포인트가 유효하지 않음"));
	        }

	        // 5. 현재 보유 포인트도 받아서 비교 (선택적)
	        int userpoint = ((Number) req.get("userpoint")).intValue();
	        if (userpoint < point) {
	            return ResponseEntity.badRequest().body(Map.of("error", "보유 포인트가 부족합니다"));
	        }

	        // 6. 서비스 호출 (포인트 차감 로직 수행)
	        int result = ps.usePoint(userid, point);

	        if (result == 1) {
	            return ResponseEntity.ok(Map.of("result", "포인트 차감 성공"));
	        } else {
	            return ResponseEntity.status(400).body(Map.of("error", "포인트 차감 실패"));
	        }
